...
const action = {
  ...
  helpinfo: (tag) => {
    return utils.color(`Command help: ${data.cmds[tag].help.replace("%tag%", tag)}`, "orange") || ""
  }
}
...
/* request api */
const request = {
  send: (url, parseJson, callback) => {
    const hosts = core.storage.getItem("allowedHosts") || ["localhost", "127.0.0.1", "api.some-random-api.com", "api.ipify.org", location.hostname];
  ...
  getHosts: () => {
    return core.storage.getItem("allowedHosts")
  }
}
...
$(document).ready(() => {
  $("#input").keyup(function(e) {
    ...
    if (e.keyCode == 13) {
      ...
      cmd.run(vals.splice(0, 1)[0].toLowerCase(), vals);
    }
    if (e.keyCode == 38) {
      $("#input").val(data.last);
    }
